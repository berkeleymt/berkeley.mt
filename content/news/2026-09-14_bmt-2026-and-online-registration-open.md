+++
title = "Registration is open for BMT 2026 and BMT 2026 Online!"
+++

We're excited to announce that registration for the 2026 Berkeley Math
Tournament is now open! The next iteration of our high school tournament will
be held on UC Berkeley campus on November 14, 2026.

The online version will be held on December 5, 2026, with an asynchronous
Power Round on a weekend before the live event.

To preserve fairness, **you may only register for one of these tournaments.**

<!-- more -->

## BMT 2026

Registration takes place in two phases, followed by late registration:

| Dates        | Registration Period | Price Per Student |
| ------------ | ------------------- | ----------------- |
| 9/14 – 9/27  | Phase 1             | $15               |
| 9/28 – 10/18 | Phase 2             | $15               |
| 10/19 – 11/1 | Late Registration   | $25               |

Phase 1 is limited to 48 seats per organization; shirts may be pre-ordered for
$8 per student during Phase 1 and Phase 2. Follow the links below for full
event details and the registration portal.

{% button_link(href="/events/bmt-2026/") %} Event Details {% end %}
{% button_link(href="https://contestdojo.com/") %} Register on ContestDojo
{% end %}

Students without a full team may use the
[BMT 2026 Teammate Finder](https://docs.google.com/spreadsheets/d/1HtisvYZ83Ksi8fm4rwvUncw2_3puw_jblvJYBfRDgsQ/edit?rm-minimal).

## BMT 2026 Online

Registration is open until November 27, 2026 at 11:59 PM PT. Follow the links
below for event details and the registration portal.

{% button_link(href="/events/bmt-2026-online/") %} Event Details {% end %}
{% button_link(href="https://contestdojo.com/") %} Register on ContestDojo
{% end %}

Students without a full team may use the
[BMT 2026 Online Teammate Finder](https://docs.google.com/spreadsheets/d/1-fJnLVGsJFmQS2oQmKKSiBh-JWlWGhk4LHVkZa0710Y/edit?rm-minimal).


## Frequently Asked Questions

See our [FAQ page](/faqs/) for answers to common questions about registration,
eligibility, and more.

To get familiar with the style and difficulty of the tournament, check out past
problems in [our archive](/archives/). Questions may be directed to
<team@berkeley.mt>.
